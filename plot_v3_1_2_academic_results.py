...
def plot_metric_distributions(
    ...
    for index, spec in enumerate(CORE_METRICS, start=1):
        ...
        ax.text(
            ...
            0.98,
            (
                f"Cliff's δ (T > F) = {delta:.3f}\n"
                f"95% bootstrap CI [{delta_low:.3f}, {delta_high:.3f}]"
            ),
            transform=ax.transAxes,
            ha="left",
            va="top",
...
def plot_auc_forest(
    ...
    table = pd.DataFrame(rows).sort_values("auc_T_higher", ascending=True)
    if table.empty:
        return [], table
    ...
    auc = table["auc_T_higher"].to_numpy(dtype=float)
    ...
    ax.set_xlabel("ROC AUC; direction fixed as T higher than F")
    ax.set_title("Descriptive discrimination of spatial and laboratory-prior evidence")
    ax.text(
        0.01,
        0.01,
        "AUC = 0.5 indicates chance-level ranking. Confidence intervals are stratified bootstrap intervals.",
        transform=ax.transAxes,
        ha="left",
        va="bottom",
        fontsize=8.5,
    ...
    return files, table.sort_values("auc_T_higher", ascending=False)
...
def plot_dataset_effect_matrix(
            ...
            rows.append(
                {
                    ...
                    "n_T": int(t_values.size),
                    "n_F": int(f_values.size),
                    "cliffs_delta_T_higher": cliffs_delta(t_values, f_values),
                }
            )
    table = pd.DataFrame(rows)
    if table.empty:
        return [], table
    pivot = table.pivot(index="dataset", columns="metric_name", values="cliffs_delta_T_higher")
    if pivot.empty:
        return [], table
    ...
    ax.set_title("Cross-dataset effect consistency: Cliff's δ, positive means T > F")
...
def write_academic_readme(
    ...
    lines: List[str] = [
        ...
        "3. 跨场景一致性：T>F 的效应是否在不同工厂数据集中保持同方向？",
        "",
        "二、主要图件",
        "- Fig01_*：每个核心指标的 T/F 分布、Cliff's delta 和 95% bootstrap CI。",
        "- Fig02_similarity_evidence_chain：原始中心→局部超额→空间门控→最终候选→剩余信号的实验室相似度变化。",
        "- Fig03_auc_forest：各指标按 T 高于 F 的固定方向计算描述性 ROC AUC。",
        "- Fig04_cross_dataset_effect_matrix：各工厂场景中的 Cliff's delta；正值表示 T>F。",
    ...
    if not distribution_stats.empty:
        for _, row in distribution_stats.iterrows():
            delta = float(row["cliffs_delta_T_higher"])
            lines.append(
                f"- {row['metric_name']}: δ={delta:.3f}, "
                f"95% CI [{row['cliffs_delta_ci_low']:.3f}, {row['cliffs_delta_ci_high']:.3f}]；"
                f"{evidence_direction_text(delta)}。"
            )
    else:
    ...
    if not effect_stats.empty:
        effect_valid = effect_stats.dropna(subset=["cliffs_delta_T_higher"])
        if not effect_valid.empty:
            consistent = (
                effect_valid.groupby("metric_name")["cliffs_delta_T_higher"]
                .apply(lambda x: float(np.mean(x > 0)))
                .sort_values(ascending=False)
            )
            lines.append("")
            lines.append("跨数据集 T>F 同方向比例：")
...
def main() -> int:
    ...
    representative_stats = pd.DataFrame()
    if args.no_representative_samples:
        ...
        if representative_stats.empty:
            skipped.append("Fig08–Fig09：未找到可读取的样本 NPZ 文件。")
        else:
