...
# 中心点背景选择：
# "minimum_energy"：保持原程序逻辑，选择8个方向中能量最低的40cm背景
# "median_spectrum"：使用各方向40cm背景频谱的逐频点中位数，更稳健
...
def get_spectrum(
    ...
    - 不再对整段WAV计算Welch平均；
    - 中心点、40cm背景点和所有偏移点必须使用完全相同的时间区间；
...
def plot_heatmap(
    ...
    save_path = os.path.join(
        result_dir,
        f"heatmap_{method}_{time_folder}_center_{center_id}.png",
    )
...
def process_single_timepoint_spectrum(
    ...
    result_dir = f"results_sh_spectrum_1s_by_center_id_{time_folder}"
    ...
    for center_id in center_ids:
        ...
        print(
            f"\n中心点净能量：{center_net_energy:.6e}，"
            f"背景来源={center_bg_name}"
        )
...
def main() -> None:
    print("=" * 80)
    print("频谱减法模式 - 50-70kHz频段分析（抗插值过冲版）")
    print("=" * 80)
    print("关键修改：")
    print("  1. 取消逐文件峰值归一化，保留不同测点真实幅值比例")
    print("  2. 主热力图使用linear插值，并限制在实测能量范围内")
    print("  3. 额外输出nearest热力图用于核对")
    print("  4. 精确解析方向文件名，避免down误匹配down_left等文件")
    print("  5. 所有泄漏判断只使用实测点，不使用插值像素")
    print("  6. center00_00只读取原始WAV的[0,1)秒")
    print("  7. center00_01只读取原始WAV的[1,2)秒，依此类推")
    print("  8. 中心和全部偏移坐标严格使用同一秒，不再整段平均")
