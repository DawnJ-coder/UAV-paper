...
def parse_offset_filename_candidates(path):
    ...
    这个格式里有两个可能的 center 编码：
        候选A：第一个数字 00 是 center
        候选B：00d5 里 d 前面的 00 是 center，d 后面的 5 是距离

    程序会在 discover_offset_files() 中统计所有文件，
    自动选择能产生更多 center、且每个 center 接近64个组合的方案。
    """
    base = os.path.basename(str(path)).lower().replace("-", "_").replace(" ", "_")
    ...
    # 如果不是这种格式，退回旧解析
    center = parse_center_from_path(path)
    direction = parse_direction_from_path(path)
    dist = parse_distance_from_path(path)

    if center is not None and direction is not None and dist is not None:
        candidates.append({
            "schema": "fallback_parser",
            "center": center,
            "direction": direction,
            "distance": int(dist),
        })
...
def choose_offset_schema(all_wav_files):
    """
    在候选解析方案中自动选择最合理的 center 解析方式。
    优先标准：
        1. unique center 数量更多；
        2. 平均每个 center 的组合数更接近 64；
        3. 总组合数更多。
    """
    schema_maps = {}

    for f in all_wav_files:
        candidates = parse_offset_filename_candidates(f)

        for c in candidates:
            schema = c["schema"]
            key = (c["center"], c["direction"], c["distance"])
    ...
    for schema, mp in schema_maps.items():
        ...
        median_per_center = float(np.median(counts)) if counts else 0.0

        # 越接近64越好，但 center数量优先
        closeness = 1.0 / (1.0 + abs(avg_per_center - 64.0))
        score = n_centers * 1000.0 + total_combos + closeness * 100.0
        ...
        score_rows.append({
            ...
            "median_per_center": median_per_center,
            "score": score,
        })

    score_rows = sorted(score_rows, key=lambda x: x["score"], reverse=True)
    best_schema = score_rows[0]["schema"]
    ...
    return best_schema, schema_maps[best_schema]
...
def parse_center_file_candidates(path):
    """
    自动解析 center wav 文件的 center 编号。

    兼容多种可能命名：
        HM20260626_142938.ld_00_beamform_result.wav
        HM20260626_142938.ld_beamform_result_00.wav
        00_beamform_result.wav
        center_00_beamform_result.wav

    这里不解析 offset 文件里的 00d5；offset 有单独解析逻辑。
    """
    base = os.path.basename(str(path)).lower().replace("-", "_").replace(" ", "_")
    candidates = []

    # 1) .ld_00_beamform_result
    m = re.search(r"\.ld_(\d{1,3})(?=_beamform|_center|_result|\.|$)", base, flags=re.IGNORECASE)
    if m:
        candidates.append(("after_ld", m.group(1).zfill(2)))

    # 2) .ld_beamform_result_00
    m = re.search(r"\.ld_.*?(?:beamform|result).*?_(\d{1,3})(?=\.|_|$)", base, flags=re.IGNORECASE)
    if m:
        candidates.append(("suffix_after_beamform", m.group(1).zfill(2)))

    # 3) center_00
    m = re.search(r"(?:center|centre|c)_(\d{1,3})(?=_|\.|$)", base, flags=re.IGNORECASE)
    if m:
    ...
    # 5) 兜底：取文件名中最后一个短数字 token，但排除 d5/d10 这种距离
...
def choose_center_schema(all_wav_files):
    """
    对 center wav 文件自动选择最合理的 center 编号解析方案。
    目标是识别出最多不同 center。
    """
    schema_maps = {}
    ...
    for f in all_wav_files:
        for schema, center in parse_center_file_candidates(f):
            schema_maps.setdefault(schema, {})
            # 一个 center 正常只有一个中心wav；如果重复，保留第一个
    ...
    print("  center解析方案候选:")
    for r in rows:
        print(f"    {r['schema']}: centers={r['n_centers']}, files={r['n_files']}")
    print("  采用center解析方案:", best_schema)
...
def discover_center_files(center_root, time_folder):
    ...
    best_schema, out = choose_center_schema(files)
...
def discover_offset_files(offset_root, time_folder):
    ...
    best_schema, out = choose_offset_schema(files)
    ...
    for ds in DATASETS:
        ...
        for time_folder in TIME_FOLDERS:
            ...
            if len(centers) > 0:
                ...
                shown = sorted(offset_count_by_center.items())[:10]
                ...
                print("  前10个center offset组合数:", shown)

                if len(offset_count_by_center) <= 2:
                    print("  [警告] 只识别到很少center，说明文件名center解析仍可能不对。")
                if avg_offset_per_center < 40:
                    print("  [警告] 平均每个center offset组合数明显小于64，请检查是否缺文件或命名特殊。")
