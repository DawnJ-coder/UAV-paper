...
OUTPUT_DIR = os.path.join(BASE_DIR, "leak_inference_results")
...
def predict_pairwise(df_a, df_b):
    ...
    rows = []
    common_centers = sorted(set(df_a["center_norm"]) & set(df_b["center_norm"]))

    for center in common_centers:
        g = both[both["center_norm"] == center].copy()
        if len(g) < 2:
            continue
    ...
    summary = {
        "mode": "pairwise_144226_rule",
        "n_common_centers": int(len(common_centers)),
        "used_rule_features": "|".join(used),
        "missing_rule_features": "|".join(missing),
        "note": "pairwise模式是一对里面分数高者判 TRUE_LEAK，不是单文件夹绝对分类。",
    }
...
def main():
    ...
    if MODE == "single":
        feature_df = extract_features_from_folder(
        ...
        pred_path = os.path.join(OUTPUT_DIR, "single_v7_predictions.csv")
        summary_path = os.path.join(OUTPUT_DIR, "single_v7_summary.csv")
        ...
        pred_path = os.path.join(OUTPUT_DIR, "pairwise_144226_predictions.csv")
        summary_path = os.path.join(OUTPUT_DIR, "pairwise_144226_summary.csv")
        pred_df.to_csv(pred_path, index=False, encoding="utf-8-sig")
        pd.DataFrame([summary]).to_csv(summary_path, index=False, encoding="utf-8-sig")
