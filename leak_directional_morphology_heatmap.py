...
def draw_direction_labels(axis: plt.Axes) -> None:
    """在圆周外标出8个方向。"""
...
def plot_absolute_morphology(
    ...
    # 实测点只作为位置参考，不画复杂椭圆和质心。
    measured_x = np.asarray(
        [record["x_cm"] for record in result["point_records"]],
        dtype=np.float64,
    )
    measured_y = np.asarray(
        [record["y_cm"] for record in result["point_records"]],
        dtype=np.float64,
    )
    ...
    # 默认不画箭头。
    if DRAW_SINGLE_DIRECTION_ARROW:
        direction_strengths = np.asarray(
            [
        ...
        if (
            direction_strengths.size > 0
            and np.max(direction_strengths) > 0
        ):
            best_index = int(np.argmax(direction_strengths))
            best_direction = result["direction_rows"][best_index]["direction"]
    ...
    dominant_text = result["dominant_directions_text"]
    title = (
        "Directional Acoustic Leak Morphology\n"
        ...
        f"Strong directions: {dominant_text}"
    )
    ...
    save_path = os.path.join(
        result["result_dir"],
        f"absolute_morphology_{result['time_folder']}"
...
def determine_global_scales(
    ...
    for result in all_results:
        profile_values = result["shape_profiles"].ravel()
...
def save_scale_info(
    ...
    with open(save_path, "w", encoding="utf-8") as file:
        file.write(
            "All absolute morphology heatmaps use the same scale.\n"
        )
...
def main() -> None:
    ...
    print("=" * 90)
    print("这版只做两件事：")
    print("  1. 输出统一色标的直观空间形态热力图")
    print("  2. 输出允许多个方向同时存在的多瓣方向图")
    print("不画大椭圆，不强制单一方向，也默认不画箭头。")
    print(
