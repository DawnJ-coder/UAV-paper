...
生成用于论文、答辩和实验报告的总体统计图，并逐一处理 samples 中的每个样本。
...
若不填写 --output-dir，则默认保存到：
    <result-dir>/academic_figures

说明
----
- 本程序不会修改 v3.1.2 的原始结果。
- 统计图默认同时保存 PNG（300 dpi）和矢量 PDF。
- samples 中每个样本保持原始 dataset/time_folder/center_id 目录层级，分别生成时频图和频谱图。
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="no_sample_plots",
        action="store_true",
        help="不遍历 samples 文件夹生成逐样本图；旧参数 --no-representative-samples 仍兼容。",
    )
...
def plot_all_sample_outputs(
    result_dir: Path,
    output_dir: Path,
    dpi: int,
    save_pdf: bool,
) -> Tuple[List[Path], pd.DataFrame]:
    """逐一处理 v3.1.2 ``samples`` 下的每个样本。

    输出保持原始层级：
        <output_dir>/samples/<dataset>/<time_folder>/center_<id>/

    因此不同工厂、不同 WAV 工况、不同中心编号不会混在同一目录，也不会只选
    T/F 各一个代表样本。
    """
    ...
    output_sample_root = output_dir / "samples"
    files: List[Path] = []
    manifest_rows: List[Dict[str, Any]] = []
    summary_paths = sorted(sample_root.rglob("summary.json"), key=lambda p: str(p).lower())
    ...
    for summary_path in summary_paths:
        try:
            ...
            manifest_rows.append({
                ...
                "status": "summary_read_error",
                "error": f"{type(exc).__name__}: {exc}",
            })
            continue

        npz_path = summary_path.parent / "spatial_local_excess_result.npz"
        try:
            relative_sample_dir = summary_path.parent.relative_to(sample_root)
        except ValueError:
            relative_sample_dir = Path(
                safe_slug(payload.get("dataset", "unknown_dataset")),
                safe_slug(payload.get("time_folder", "root") or "root"),
                f"center_{safe_slug(payload.get('center_id', 'unknown'))}",
            )
        sample_output_dir = output_sample_root / relative_sample_dir
        sample_output_dir.mkdir(parents=True, exist_ok=True)

        metadata_path = sample_output_dir / "00_sample_metrics.csv"
        pd.DataFrame([{k: _json_scalar(v) for k, v in payload.items()}]).to_csv(
        ...
        base_row: Dict[str, Any] = {
            ...
            "time_folder": payload.get("time_folder", ""),
            "canonical_condition": payload.get("canonical_condition", ""),
            "center_id": payload.get("center_id", ""),
            "label": normalize_label(payload.get("label", "")),
            "center_file": payload.get("center_file", ""),
            "source_center_score": payload.get("source_center_score", np.nan),
            "source_sample_directory": str(summary_path.parent),
            "output_sample_directory": str(sample_output_dir),
            "npz_path": str(npz_path),
        }

        if not npz_path.is_file():
            manifest_rows.append({**base_row, "status": "missing_npz", "n_generated_files": 1})
            continue
        ...
        try:
            ...
            raw_reference: Optional[float] = None
            ...
            sample_text = (
                ...
                f"type={payload.get('time_folder', '')} | "
                f"center={payload.get('center_id', '')} | "
                f"label={normalize_label(payload.get('label', ''))}"
            )

            for index, (key, title_name, colorbar_label, fixed_range) in enumerate(
                NPZ_HEATMAPS, start=1
            ):
                if key not in data:
                    continue
                ...
                files.extend(
                    plot_single_tf_heatmap(
                        ...
                        f"{title_name}\n{sample_text}",
                        colorbar_label,
                        sample_output_dir / f"Fig08_{index:02d}_{safe_slug(key)}",
                        dpi,
                        save_pdf,
            ...
            if available_spectra:
                ...
                ax.set_title(f"Spectral separation chain\n{sample_text}")
                ax.legend(frameon=False)
                fig.tight_layout()
                files.extend(
                    save_figure(
                        fig,
                        sample_output_dir / "Fig09_spectral_separation_chain",
                        dpi,
                        save_pdf,
                    )
                )

            source_note = sample_output_dir / "00_source_sample.txt"
            source_note.write_text(
                "\n".join([
                    f"source_sample_directory={summary_path.parent}",
                    f"source_summary={summary_path}",
                    f"source_npz={npz_path}",
                    f"center_file={payload.get('center_file', '')}",
                    f"dataset={payload.get('dataset', '')}",
                    f"time_folder={payload.get('time_folder', '')}",
                    f"canonical_condition={payload.get('canonical_condition', '')}",
                    f"center_id={payload.get('center_id', '')}",
                    f"label={normalize_label(payload.get('label', ''))}",
                ]),
                encoding="utf-8",
            )
            files.append(source_note)
            manifest_rows.append({
                **base_row,
                "status": "generated",
                "n_generated_files": len(files) - before_count + 1,
            })
        except Exception as exc:
            manifest_rows.append({
                **base_row,
    ...
    return files, pd.DataFrame(manifest_rows)
...
def write_academic_readme(
    ...
    lines: List[str] = [
        ...
        "- samples/<原始样本层级>/Fig08_*：每一个 WAV 样本各自的时频证据矩阵。",
        "- samples/<原始样本层级>/Fig09_*：每一个 WAV 样本各自的频谱分离链。",
        "",
        "三、自动统计摘要",
    ]
...
def main() -> int:
    ...
    output_dir = (
        ...
        else result_dir / "academic_figures"
    )
    ...
    if distribution_stats.empty:
        skipped.append("Fig01：缺少可用的核心 T/F 指标。")
    else:
    ...
    if auc_stats.empty:
        skipped.append("Fig03：无法计算 T/F AUC。")
    else:
    ...
    joint_files = plot_joint_evidence_map(
        summary,
        output_dir,
        args.dpi,
        save_pdf,
    ...
    if paired_stats.empty:
        skipped.append("Fig06：没有可匹配的 T/F pair_key。")
    else:
    ...
    if distance_stats.empty:
        skipped.append("Fig07：11_opposite_pair_diagnostics.csv 缺失或字段不足。")
    else:
        path = output_dir / "Table07_opposite_pair_distance_statistics.csv"
        write_table(distance_stats, path)
        generated.append(path)

    sample_plot_stats = pd.DataFrame()
    if args.no_sample_plots:
        skipped.append("samples/Fig08–Fig09：用户通过参数关闭逐样本图。")
    else:
        sample_files, sample_plot_stats = plot_all_sample_outputs(
            result_dir,
            output_dir,
            args.dpi,
            save_pdf,
        )
        generated.extend(sample_files)
        if sample_plot_stats.empty:
            skipped.append("samples/Fig08–Fig09：未找到 samples/summary.json 或可读取的样本 NPZ。")
        else:
            path = output_dir / "Table08_all_sample_plots.csv"
            write_table(sample_plot_stats, path)
            generated.append(path)
    ...
    print("=" * 88)
    print("v3.1.2 学术结果图生成完成")
    print(f"结果目录：{result_dir}")
    print(f"图件目录：{output_dir}")
    print(f"有效样本：{len(summary)}")
    print(f"生成文件：{len(generated)}")
    print(f"逐样本图目录：{output_dir / 'samples'}")
    print(f"先阅读：{readme}")
    if skipped:
        print("以下图件因输入不足被跳过：")
        for item in skipped:
