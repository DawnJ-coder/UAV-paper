...
------------
1. A场景内部验证并建立最终冻结指纹：
       python leak_v10_fingerprint_similarity_blocked_seconds.py

2. 自检：
       python leak_v10_fingerprint_similarity_blocked_seconds.py --self-test

3. 指定 v9 结果目录：
       python leak_v10_fingerprint_similarity_blocked_seconds.py --v9-dir "D:\\xxx\\leak_v9_local_background_results"
...
重要说明
--------
- 程序启动后先解析 v9_all_features.csv 和 center_file 文件名。
- 支持 00_00、00_01、00_02……格式；00_00按第1秒处理，内部索引为0。
- 连续若干秒组成一个时间块，整块留出验证，避免相邻秒随机分到训练和测试。
- 文件名解析失败时才尝试 time 分组；最后才退回样本级分层验证，并明确警告。
...
# 秒编号解析与连续时间块验证
# -----------------------------------------------------------------------------
# 例如 00_00~00_04 为第1块，00_05~00_09 为第2块。
TIME_BLOCK_SECONDS = 5

# v9旧版可能把 00_00 只保存成 center=0。此时允许把纯数字center解释成从0开始的秒索引。
NUMERIC_CENTER_IS_ZERO_BASED_SECOND = True

# 打印/保存多少条文件名解析示例。
TIME_PARSE_PREVIEW_ROWS = 20
...
class V10Config:
    ...
    time_block_seconds: int = TIME_BLOCK_SECONDS
    numeric_center_is_zero_based_second: bool = NUMERIC_CENTER_IS_ZERO_BASED_SECOND
    time_parse_preview_rows: int = TIME_PARSE_PREVIEW_ROWS
...
class Fingerprint:
    ...
    temporal: np.ndarray
    spatial: np.ndarray
    parsed_time_token: str = ""
    second_index: Optional[int] = None
    second_number: Optional[int] = None
    time_parse_source: str = ""
    time_parse_rule: str = ""
    second_block_index: Optional[int] = None
    second_block_id: str = ""
...
def _parse_mm_ss_token(token: str) -> Optional[Tuple[int, str, str]]:
    """
    解析 MM_SS：00_00 -> 秒索引0（第1秒），00_01 -> 秒索引1。
    允许分钟超过59；秒必须在0~59。
    """
    m = re.fullmatch(r"(?P<minute>\d{1,6})_(?P<second>\d{1,2})", token.strip())
    if not m:
        return None
    ...
    normalized = f"{minute:02d}_{second:02d}"
    return total, normalized, "MM_SS"


def _parse_hh_mm_ss_token(token: str) -> Optional[Tuple[int, str, str]]:
    m = re.fullmatch(
        r"(?P<hour>\d{1,3})_(?P<minute>\d{1,2})_(?P<second>\d{1,2})",
        token.strip(),
    )
    if not m:
        return None
    ...
    normalized = f"{hour:02d}_{minute:02d}_{second:02d}"
    return total, normalized, "HH_MM_SS"


def parse_second_from_center_file(value: Any) -> Optional[Dict[str, Any]]:
    """优先从原中心WAV文件名末尾解析时间标记。"""
    text = _clean_text(value)
    if not text:
        return None
    name = Path(text).name
    stem = re.sub(r"_beamform_result\.wav$", "", name, flags=re.IGNORECASE)
    if stem == name:
        return None

    # 优先识别最后三个数字字段；若不满足合法时分秒，再识别最后两个字段。
    tokens = stem.split("_")
    if len(tokens) >= 3:
        parsed = _parse_hh_mm_ss_token("_".join(tokens[-3:]))
        if parsed is not None:
            total, token, rule = parsed
            return {
                "second_index": total,
                "parsed_time_token": token,
                "time_parse_source": "center_file",
                "time_parse_rule": rule + "_before_beamform_suffix",
            }
    if len(tokens) >= 2:
        parsed = _parse_mm_ss_token("_".join(tokens[-2:]))
        if parsed is not None:
            total, token, rule = parsed
            return {
                "second_index": total,
...
def parse_second_from_center(value: Any, allow_numeric: bool) -> Optional[Dict[str, Any]]:
    text = _clean_text(value)
    if not text:
        return None

    parsed3 = _parse_hh_mm_ss_token(text)
    if parsed3 is not None:
        total, token, rule = parsed3
        return {
            "second_index": total,
            "parsed_time_token": token,
            "time_parse_source": "center",
            "time_parse_rule": rule,
        }

    parsed2 = _parse_mm_ss_token(text)
    if parsed2 is not None:
        total, token, rule = parsed2
        return {
            "second_index": total,
            "parsed_time_token": token,
            "time_parse_source": "center",
    ...
    if allow_numeric and re.fullmatch(r"\d+", text):
        ...
        return {
            "second_index": total,
            "parsed_time_token": str(total),
            "time_parse_source": "center",
            "time_parse_rule": "numeric_zero_based_second",
        }
    return None


def parse_second_from_sample_id(value: Any, allow_numeric: bool) -> Optional[Dict[str, Any]]:
    text = _clean_text(value)
    if not text:
        return None
    m = re.search(r"center_(?P<center>\d+(?:_\d+){0,2})$", text, flags=re.IGNORECASE)
    if not m:
        return None
    result = parse_second_from_center(m.group("center"), allow_numeric=allow_numeric)
    if result is not None:
        result["time_parse_source"] = "sample_id"
        result["time_parse_rule"] = result["time_parse_rule"] + "_after_center"
    return result


def parse_one_metadata_row(row: pd.Series, config: V10Config) -> Dict[str, Any]:
    """
    解析优先级：center_file完整文件名 > center列 > sample_id。
    原因：v9旧版可能把 00_00 简化成 center=0，而center_file仍保留完整文件名。
    """
    attempts = [
        parse_second_from_center_file(row.get("center_file", "")),
        parse_second_from_center(
            row.get("center", ""),
            allow_numeric=config.numeric_center_is_zero_based_second,
        ),
        parse_second_from_sample_id(
            row.get("sample_id", ""),
            allow_numeric=config.numeric_center_is_zero_based_second,
        ),
    ]
    parsed = next((x for x in attempts if x is not None), None)
    if parsed is None:
        return {
            "time_parse_ok": 0,
            "parsed_time_token": "",
            "second_index": np.nan,
            "second_number": np.nan,
            "time_parse_source": "",
            "time_parse_rule": "unparsed",
            "second_block_index": np.nan,
            "second_block_id": "",
        }

    second_index = int(parsed["second_index"])
    block_size = max(int(config.time_block_seconds), 1)
    block_index = second_index // block_size
    return {
        "time_parse_ok": 1,
        "parsed_time_token": str(parsed["parsed_time_token"]),
        "second_index": second_index,
        "second_number": second_index + 1,
        "time_parse_source": str(parsed["time_parse_source"]),
        "time_parse_rule": str(parsed["time_parse_rule"]),
        "second_block_index": block_index,
        "second_block_id": f"second_block_{block_index:04d}",
    }


def read_and_parse_v9_metadata(
    v9_dir: Path,
    config: V10Config,
    ...
    metadata = pd.read_csv(
        csv_path,
        dtype={"center": str, "sample_id": str, "time": str, "center_file": str},
    )
    ...
    parsed_rows = [parse_one_metadata_row(row, config) for _, row in metadata.iterrows()]
    parsed_df = pd.DataFrame(parsed_rows, index=metadata.index)
    for col in parsed_df.columns:
        metadata[col] = parsed_df[col]
    ...
    report_df = metadata[report_cols].copy()

    n_total = len(report_df)
    n_ok = int(report_df["time_parse_ok"].sum())
    lines = [
        "v10.1 文件格式与秒编号解析报告",
        "=" * 88,
        f"输入CSV: {csv_path}",
        f"总样本数: {n_total}",
        f"成功解析秒编号: {n_ok}",
        f"解析成功率: {n_ok / max(n_total, 1):.4f}",
        f"连续时间块大小: {config.time_block_seconds} 秒",
    ...
    if n_ok:
        lines.append("解析来源统计:")
        source_counts = report_df.loc[report_df["time_parse_ok"] == 1, "time_parse_source"].value_counts()
        for key, value in source_counts.items():
            lines.append(f"  {key}: {int(value)}")
        lines.append("")
        lines.append("各标签秒编号/时间块统计:")
        for label, sub in report_df[report_df["time_parse_ok"] == 1].groupby("label"):
            sec_min = int(pd.to_numeric(sub["second_index"]).min())
            sec_max = int(pd.to_numeric(sub["second_index"]).max())
            blocks = sorted(sub["second_block_id"].astype(str).unique().tolist())
            lines.append(
                f"  {label}: n={len(sub)}, 秒索引范围={sec_min}~{sec_max}, "
                f"时间块数={len(blocks)}, 时间块={blocks}"
            )
    if n_ok < n_total:
        lines.extend([
            "",
            "未解析样本存在。程序仍会继续，但这些样本不能参加连续秒分块验证，",
            "若参考scene无法形成至少2个TRUE块和2个FALSE块，将尝试旧time分组或样本级回退。",
        ])
...
def load_fingerprints(
    ...
    if metadata is None:
        metadata, _, _ = read_and_parse_v9_metadata(v9_dir, config)
    else:
        ...
        try:
            ...
            fingerprints.append(
                Fingerprint(
                    ...
                    temporal=temporal,
                    spatial=spatial,
                    parsed_time_token=str(row.get("parsed_time_token", "")),
                    second_index=(
                        int(row["second_index"])
                        if pd.notna(row.get("second_index", np.nan))
                        else None
                    ),
                    second_number=(
                        int(row["second_number"])
                        if pd.notna(row.get("second_number", np.nan))
                        else None
                    ),
                    time_parse_source=str(row.get("time_parse_source", "")),
                    time_parse_rule=str(row.get("time_parse_rule", "")),
                    second_block_index=(
                        int(row["second_block_index"])
                        if pd.notna(row.get("second_block_index", np.nan))
                        else None
                    ),
                    second_block_id=str(row.get("second_block_id", "")),
                )
            )
        except Exception as exc:
            failures.append(
        ...
        summary_rows.append(
            {
                ...
                "parsed_time_token": fp.parsed_time_token,
                "second_index": fp.second_index,
                "second_number": fp.second_number,
                "time_parse_source": fp.time_parse_source,
                "time_parse_rule": fp.time_parse_rule,
                "second_block_index": fp.second_block_index,
                "second_block_id": fp.second_block_id,
...
def make_cv_splits(
    ...
    # ------------------------------------------------------------------
    # 1. 首选：解析 00_00 / 00_01 等秒编号后，按连续时间块整组留出。
    # ------------------------------------------------------------------
    parsed_mask = np.asarray(
        [fp.second_index is not None and bool(fp.second_block_id) for fp in samples],
        dtype=bool,
    )
    if np.all(parsed_mask):
        second_groups = np.asarray([fp.second_block_id for fp in samples], dtype=str)
        true_blocks = set(second_groups[labels == "TRUE_LEAK"])
        false_blocks = set(second_groups[labels == "FALSE_LEAK"])
        common_blocks = sorted(true_blocks & false_blocks)

        if len(true_blocks) >= 2 and len(false_blocks) >= 2 and len(common_blocks) >= 2:
            logo = LeaveOneGroupOut()
            splits: List[Tuple[np.ndarray, np.ndarray]] = []
            skipped: List[str] = []
            for train_idx, test_idx in logo.split(np.arange(len(samples)), y, second_groups):
                test_group = str(second_groups[test_idx[0]])
                # 建模集必须有两类；测试块优先要求两类都有，便于每折解释。
                if len(np.unique(y[train_idx])) < 2:
                    skipped.append(test_group + ":训练集缺类别")
                    continue
                if len(np.unique(y[test_idx])) < 2:
                    skipped.append(test_group + ":测试块缺类别")
                    continue
                splits.append((train_idx, test_idx))
            if splits:
                if skipped:
                    warnings.append("部分秒时间块未参加验证: " + " | ".join(skipped))
                warnings.append(
                    f"已从文件名/center解析秒编号，并按每{config.time_block_seconds}秒一个连续块整组留出。"
                    "同一时间块内的相邻秒不会同时出现在训练集和测试集。"
                )
                return "blocked_seconds_cv", splits, warnings
        warnings.append(
            "秒编号解析成功，但TRUE/FALSE没有形成至少2个共同连续时间块，无法执行blocked_seconds_cv。"
        )
    else:
        n_bad = int(np.sum(~parsed_mask))
        warnings.append(f"有{n_bad}个参考样本未成功解析秒编号，无法对全部样本做连续秒分块验证。")

    # ------------------------------------------------------------------
    # 2. 次选：真正存在多个独立time目录时，按time整组留出。
    # ------------------------------------------------------------------
    time_groups = np.asarray([fp.time for fp in samples], dtype=str)
    true_groups = set(time_groups[labels == "TRUE_LEAK"])
    false_groups = set(time_groups[labels == "FALSE_LEAK"])
    splits = []
    if len(true_groups) >= 2 and len(false_groups) >= 2 and len(np.unique(time_groups)) >= 3:
        logo = LeaveOneGroupOut()
        for train_idx, test_idx in logo.split(np.arange(len(samples)), y, time_groups):
            if len(np.unique(y[train_idx])) < 2:
                continue
            splits.append((train_idx, test_idx))
        if splits:
            warnings.append("未使用秒分块，改为按v9的time列整组留出。")
            return "leave_one_time_out", splits, warnings

    # ------------------------------------------------------------------
    # 3. 最后回退：样本级分层CV。仅用于发现程序/数据是否有明显信号。
    # ------------------------------------------------------------------
    ...
    warnings.append(
        "无法进行连续秒分块或独立time验证，当前退回分层样本交叉验证；"
        "该结果只能作为初步检查。"
    )
    ...
    for fold_index, (train_idx, test_idx) in enumerate(splits, start=1):
        ...
        for sample in test_samples:
            ...
            row = {
                ...
                "parsed_time_token": sample.parsed_time_token,
                "second_index": sample.second_index,
                "second_number": sample.second_number,
                "second_block_index": sample.second_block_index,
                "second_block_id": sample.second_block_id,
            ...
            fold_rows.append(
                {
                    ...
                    "test_second_blocks": " | ".join(
                        sorted(set(fold_df.get("second_block_id", pd.Series(dtype=str)).astype(str)))
                    ),
                    "test_second_range": (
                        f"{int(fold_df['second_index'].min())}~{int(fold_df['second_index'].max())}"
                        if "second_index" in fold_df.columns and fold_df["second_index"].notna().any()
                        else ""
                    ),
                    "n_test": len(fold_df),
...
def make_pairwise_similarity_matrix(
    ...
    labels = [
        f"{fp.label[:1]}|{fp.scene}|sec{fp.second_index if fp.second_index is not None else fp.center}"
        for fp in selected
    ]
    ...
    index_df = pd.DataFrame(
        {
            ...
            "parsed_time_token": [fp.parsed_time_token for fp in selected],
            "second_index": [fp.second_index for fp in selected],
            "second_block_id": [fp.second_block_id for fp in selected],
        }
    )
...
def make_report(
    ...
    if internal_summary:
        ...
        lines.append("  4. 优先看blocked_seconds_cv；它按连续秒块整组留出。leave_one_time_out更严格；样本级回退只能作为初步检查。")
        lines.append("")
    ...
    lines.append("  - 同一文件不同连续秒块可验证文件内时间稳定性；最终仍需用独立录音/场景验证跨采集稳定性。")
...
def run_all_mode(
    ...
    # 先解析CSV/中心文件名中的秒编号，再读取NPZ和构造指纹。
    parsed_metadata, time_parse_df, time_parse_text = read_and_parse_v9_metadata(
        v9_dir, config
    )
    time_parse_path = output_dir / "v10_time_parse_report.csv"
    time_parse_summary_path = output_dir / "v10_time_parse_summary.txt"
    time_parse_df.to_csv(time_parse_path, index=False, encoding="utf-8-sig")
    with open(time_parse_summary_path, "w", encoding="utf-8") as f:
        f.write(time_parse_text)

    print("=" * 90)
    print("v10.1 第一步：解析中心文件格式和秒编号")
    print("=" * 90)
    print(time_parse_text)
    preview_cols = [
        c for c in [
            "label", "time", "center", "center_file", "parsed_time_token",
            "second_index", "second_number", "second_block_id", "time_parse_source",
        ] if c in time_parse_df.columns
    ]
    if preview_cols:
        print("\n解析示例:")
        print(time_parse_df[preview_cols].head(config.time_parse_preview_rows).to_string(index=False))

    fingerprints, fingerprint_index, failure_df = load_fingerprints(
        v9_dir, config, metadata=parsed_metadata
    )
    ...
    return {
        ...
        "time_parse": time_parse_path,
        "time_parse_summary": time_parse_summary_path,
    }
...
def run_external_mode(
    ...
    print(time_parse_text)
    fingerprints, fingerprint_index, failure_df = load_fingerprints(
        v9_dir, bundle.config, metadata=parsed_metadata
    )
...
def run_self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="v10_self_test_") as tmp:
        ...
        for scene, shift in [("factory_A", 0.0), ("factory_B", 1500.0)]:
            for label in ["TRUE_LEAK", "FALSE_LEAK"]:
                time_name = f"{scene}_single_recording_{label}"
                for second_index in range(15):
                    center += 1
                    mm = second_index // 60
                    ss = second_index % 60
                    token = f"{mm:02d}_{ss:02d}"
                    sample_id = f"{scene}_{label}_{time_name}_center_{second_index}"
        ...
        split_mode = str(summary.loc[0, "split_mode"])
        if split_mode != "blocked_seconds_cv":
            raise AssertionError(f"自检未使用blocked_seconds_cv，而是: {split_mode}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--time-block-seconds",
        type=int,
        default=TIME_BLOCK_SECONDS,
        help="连续多少秒组成一个验证块，默认5",
    )
...
def main() -> None:
    ...
    if args.time_block_seconds <= 0:
        raise ValueError("--time-block-seconds 必须大于0")
    ...
    if args.mode == "external":
        ...
        config = V10Config(time_block_seconds=int(args.time_block_seconds))
        run_all_mode(v9_dir, output_dir, args.reference_scene, config)
