...
def train(leak_files, noise_files, **kw):
    ...
    win_tau = float(np.percentile(dl, 90))            # 仅用于算 frac(显示)

    lst = [file_stats(det, f, win_tau) for f in leak if len(f)]
    nst = [file_stats(det, f, win_tau) for f in noise if len(f)]
    lp90 = np.array([s['p90'] for s in lst]); np90 = np.array([s['p90'] for s in nst])
    lmed = np.array([s['median'] for s in lst])

    # 主判据 p90: 两类间隔中点; 护栏 median: 泄漏上限留 15% 余量(宽松)
    tau_p90 = _gap_threshold(lp90, np90, "p90  ")
    tau_med = float(np.max(lmed) * 1.15)
    print(f"[train] => TAU_P90={tau_p90:.2f}  TAU_MED={tau_med:.2f}(护栏)  win_tau={win_tau:.2f}")
    return det, dict(win_tau=win_tau, tau_p90=tau_p90, tau_med=tau_med)
...
def evaluate(det, files, label, params, verbose=True, **kw):
    ...
    for p, fr in zip(files, frames):
        ...
        is_leak = decide_file(st, params['tau_p90'], params['tau_med'])
